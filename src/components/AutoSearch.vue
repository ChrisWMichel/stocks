  ...
  methods: {
    fetchResults() {
      ...
      this.timeout = setTimeout(() => {
        const usersPromise = axios.get(`https://cloud.iexapis.com/stable/search/${this.query}?token=pk_b126cb1ba8dc441aa3673f3ccb2a3ba5`);
        ...
        Promise.all([usersPromise]).then(resp => {
         ...
         // const symbol = this.filterResults(stockList.symbol, query);
          ...
          stockList.length && this.suggestions.push({ name: "securityName", data: stockList});
        });
